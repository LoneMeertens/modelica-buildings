...
model EN12975 "Model of a solar thermal collector according to the ASHRAE Standard 93"
  ...
  Buildings.Fluid.SolarCollectors.PVT.BaseClasses.EN12975SolarGain solGai(
  ...
  Buildings.Fluid.SolarCollectors.PVT.BaseClasses.EN12975HeatLoss heaLos(
    redeclare package Medium = Medium,
    final nSeg=nSeg,
    final a1=per.a1,
    final a2=per.a2,
  ...
  Modelica.Blocks.Interfaces.RealOutput T_cell(quantity="Temperature", unit="K") "Cell temperature of PV panel"
...
First implementation.
      ...
      graphics={
        ...
        Line(
          ...
          origin={0,40},
          rotation=90)}),
    __Dymola_DymolaStoredErrors(thetext="model EN12975 \"Model of a solar thermal collector according to the ASHRAE Standard 93\"
  extends Buildings.Fluid.SolarCollectors.BaseClasses.PartialSolarCollector(
    redeclare Buildings.Fluid.SolarCollectors.Data.GenericEN12975 per);
  parameter Modelica.Units.SI.CoefficientOfHeatTransfer U_pvt=15 \"Heat coefficient used in cell temperature calculation\";
  ...
  Modelica.Blocks.Interfaces.RealOutput T_cell(quantity=\"Temperature\", unit=\"K\") \"Cell temperature of PV panel\"
    annotation (Placement(transformation(extent={{100,-70},{120,-50}})));
  Modelica.Units.SI.Temperature Tm \"Mean temperature\";
...
equation 
  ...
  assert(per.a1 > 0,
    \"In \" + getInstanceName() + \": The heat loss coefficient from the EN 12975 ratings data must be strictly positive. Obtained a1 = \" + String(per.a1));
  Tm = T[1]+T[nSeg]/2;
  qth = sum(QLos)+sum(Qsupply);
  T_cell = vgl voor Tcell;
